...
import { StyleSheet, StatusBar, ScrollView, SafeAreaView, Image, View, Text, TouchableOpacity, FlatList } from 'react-native';
import { heightPercentageToDP as hp, widthPercentageToDP as wp  } from 'react-native-responsive-screen';
import { RFPercentage } from 'react-native-responsive-fontsize';
import { Card, Input, Spinner } from './common';
import { Icon } from 'native-base';
import { connect } from 'react-redux';
import { mentorshipListings } from '../actions/mentorshipListingsAction';
import { searchAllListings } from '../actions/searchAllListingsAction';

class MentorshipListings extends Component {

  navigateTo = (screen, params) => this.props.navigation.navigate(screen, params);

  componentDidMount = async ()  => {
    await this.props.mentorshipListings();
    this.renderData();
  }
  ...
  */
  ...
  render() {
    ...
    const Item = ({
      ...
      <>
        {
          buttonPress && searchTerm && bucket.name !== 'Mentorship' ? (
          <Text style={noListingTextStyle}>
            No Listing under Mentorship for {searchTerm}
          </Text>
          ) : (
            <TouchableOpacity
            onPress={() => this.navigateTo('ListingDetails', {
              listingDetails: {
                imageUrl,
                title,
                tag: tag.title,
        ...
        <SafeAreaView style={{ flex: 1 }}>
          <View style={topbarViewStyle}>
            ...
            <ScrollView
              ...
              <TouchableOpacity
                onPress={() => this.navigateTo('Listings', {transition: 'fadeIn'})}
                style={touchableOpacityTextStyle}>
                <Text style={textBarStyle}>
                  All
                </Text>
              </TouchableOpacity>

              <TouchableOpacity
                onPress={() => this.navigateTo('EducationListings', {transition: 'fadeIn'})}
                style={touchableOpacityTextStyle}>
                <Text style={textBarStyle}>
                  Education
                </Text>
              ...
              <TouchableOpacity
                onPress={() => this.navigateTo('EmploymentListings', {transition: 'fadeIn'})}
                style={touchableOpacityTextStyle}>
                <Text style={textBarStyle}>
                  Employment
                </Text>
              ...
              <TouchableOpacity
                onPress={() => this.navigateTo('Favorited', {transition: 'fadeIn'})}
                style={touchableOpacityTextStyle}>
                <Text style={textBarStyle}>
                  Favourites
                </Text>
          ...
          <Card style={cardStyle}>
            {
              loadingListing || loadingSearch ? (
                ...
                />
                  ...
                  <>
                    {
                      buttonPress && searchTerm && this.props.searchData.length < 1 ? (
                      <Text style={noListingTextStyle}>No Listing for {searchTerm}</Text>
                      ) : (
                        <>
                          {
                            !buttonPress ? (
...
const mapStateToProps = state => ({
  loadingListing: state.mentorshipListingsReducer.loading,
  data: state.mentorshipListingsReducer.data,
  searchData: state.searchAllListingReducer.data,
  loadingSearch: state.searchAllListingReducer.loading,
});

export default connect(mapStateToProps, { mentorshipListings, searchAllListings })(MentorshipListings);
